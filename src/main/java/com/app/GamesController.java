...
public class GamesController implements Initializable{
	...
	public static int cbEnabled = 0; // color blind enabled = 1, disabled = 0;
	
    //create scene to begin game
	public void switchToBlockBreaker(ActionEvent event)throws IOException {
		if(cbEnabled == 1) {
			FXMLLoader fxmlLoader = new FXMLLoader();
			String pathToFxml = "src/main/resources/ColorBlindBlockBreaker.fxml";		
	...
	public void switchToSnake(ActionEvent event) throws IOException{
		if(cbEnabled == 1) {
			FXMLLoader fxmlLoader = new FXMLLoader();
			String pathToFxml = "src/main/resources/ColorBlindSnake.fxml";
			URL fxmlUrl = new File(pathToFxml).toURI().toURL();
			fxmlLoader.setLocation(fxmlUrl);
